...
function GetDataSourceNameFromQueryString() {
  ...
  if (!DataSourceName) console.log("Nothing to print as datasource");
  if (DataSourceName) return DataSourceName;
  else return "FailedDataSource";
}
...
function generateScorecard(
  ...
  const arrowClass =
    actual > target ? "fa-arrow-up up-icon" : "fa-arrow-down down-icon";
  scorecard.innerHTML = `
  ...
  const chart = {
    ...
    width: width,
    height: 400,
    left: resizableCard.getPosition().left,
    top: resizableCard.getPosition().top,
  };
  itemList.push(chart);
}
...
function GetApiUrl(dataSource) {
  const apiUrl =
    "http://localhost:5006/api/DataGateway/GetGeneratedDataDynamicColumns";
...
async function InsertChart() {
  ...
  if (chartData) {
    ...
    const chart = {
      ...
      type: chartType,
      dataSource: chartData,
      xattribute: xaxisvalue,
      yattribute: yaxisvalue,
...
function updateChart(chartContainer, newWidth, newHeight) {
  ...
  const indexToUpdate = itemList.findIndex(
    (item) => item.id === mainContainer.id
  );
  if (indexToUpdate !== -1) {
    ...
    BuildChart(
      chartContainer,
      itemToUpdate.dataSource,
      itemToUpdate.type,
      chartContainer.id,
      itemToUpdate.width,
      itemToUpdate.height,
      itemToUpdate.xattribute,
      itemToUpdate.yattribute
    );
  }
}

function CreateTreeMapChart(chartWidth, chartHeight, chartData, g) {
  const treemap = d3.treemap().size([chartWidth, chartHeight]).padding(2);

  const root = d3.hierarchy({ children: chartData }).sum((d) => d.value); //TODO: this should be change to use attribute xScale,xAttribute, yScale,yAttribute instead
...
function CreatePieChart(
  ...
  const pie = d3.pie().value((d) => yScale(d[yAttribute])); //TODO: this should be change to use attribute xScale,xAttribute, yScale,yAttribute instead
...
function adjustEditorDimensions() {
  ...
  const totalWidth = children.reduce(
    (width, element) => Math.max(width, element.offsetWidth),
    0
  );
...
async function populateDropDownList() {
  {
    ...
    await getMetaData(apiUrl, dataSourceName).then((data) => {
      if (data) {
...
async function PredefineChart(chartTyp) {
  ...
  //const dataSourceName = GetDataSourceNameFromQueryString()
  const apiURL =
    "http://localhost:5006/api/DataGateway/GetGeneratedDataDynamicColumns?dataSourceName=CGSEDW2023PG&SelectedTables=dimgeography&SelectedTables=factinternetsales&SelectedColumns[dimgeography]=englishcountryregionname&SelectedColumns[factinternetsales]=sum_of_sales_amount";
